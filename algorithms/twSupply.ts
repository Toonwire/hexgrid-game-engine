...
function turn(myCells: PlayerCell[]): PlayerTransaction {
  function isSafe(cell: PlayerCell) {
    return !cell.neighbors.some((n) => n.owner === HexOwner.OTHER);
  }

  function countSafeCells(cells: PlayerCell[]) {
    let count = 0;
  ...
  function nearbyFriends(neighbor: NeighborCell, cells: PlayerCell[]) {
    let friends = 0;
    for (var i = 0; i < cells.length; i++) {
  ...
  function hasFreeNones(cells: PlayerCell[]) {
    for (var i = 0; i < cells.length; i++) {
      for (var j = 0; j < cells[i].neighbors.length; j++) {
  ...
  function getLargestCluster(cells: PlayerCell[]) {
    var assignedCells = {};
    var clusters = {};
    var clusterCount = 0;
    var cluster = 0;
